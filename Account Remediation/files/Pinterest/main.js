...
function onEvent(arr, i) { 
	...
	if (anchors[i].text != null) {
		anchorName = anchors[i].text;
    }
            
	//Then checks whether the anchor has a title
            
	else if (anchors[i].title != null) {
		anchorName = anchors[i].title;
    }
			
	anchorName = anchorName.replace(regex, "");
			
	if (exists(arr, anchorName)) {
		for (var k = 0; k < arr.length; k++) {
			if(arr[k][0] == anchorName) {
				arr[k][1]++;
				break;
			}
		}
